# FMP 代码修复计划（2026-01-14）

## 修复任务列表

| # | 问题 | 优先级 | 状态 | 涉及文件 |
|---|------|--------|------|---------|
| 1 | 播放时不验证本地文件存在性 | 🔴 高 | ⏳ 待修复 | audio_provider.dart, queue_manager.dart |
| 2 | `_getDownloadBaseDir` 实现重复 | 🔴 高 | ⏳ 待修复 | download_path_utils.dart, 4个服务文件 |
| 3 | `cachedPath` 字段未使用 | 🟡 中 | ⏳ 待修复 | track.dart, audio_provider.dart, queue_manager.dart |
| 4 | `localCoverPath` 同步 I/O | 🟡 中 | ⏳ 待修复 | track_extensions.dart |
| 5 | 数据源不一致 | 🟢 低 | ✅ 可接受 | - |
| 6 | "正在播放"判断不一致 | 🟢 低 | ⏳ 待修复 | downloaded_category_page.dart |

---

## 修复 1：播放时验证本地文件存在性

### 问题描述
`track.firstDownloadPath` 只返回 `downloadPaths[0]`，不检查文件是否实际存在。如果用户手动删除了文件，会导致播放失败。

### 修复方案
在 `DownloadStatusCache` 中添加同步方法，遍历所有路径找到第一个存在的文件。

### 修改文件

#### 1. `lib/providers/download/download_status_cache.dart`
已有 `getFirstExistingPathSync()` 方法，但它不更新缓存。需要确保在播放时正确使用。

#### 2. `lib/services/audio/audio_provider.dart`

**修改位置**: `_playTrack()` 方法（约 843 行）

```dart
// 旧代码
final url = trackWithUrl.firstDownloadPath ??
            trackWithUrl.cachedPath ??
            trackWithUrl.audioUrl;

if (trackWithUrl.firstDownloadPath != null || trackWithUrl.cachedPath != null) {
  await _audioService.playFile(url);
}

// 新代码
String? localPath;
// 遍历所有下载路径，找到第一个存在的文件
for (final path in trackWithUrl.downloadPaths) {
  if (await File(path).exists()) {
    localPath = path;
    break;
  }
}

final url = localPath ?? trackWithUrl.audioUrl;

if (localPath != null) {
  await _audioService.playFile(url!);
} else {
  // 网络播放
  final headers = _getHeadersForTrack(trackWithUrl);
  await _audioService.playUrl(url!, headers: headers);
}
```

#### 3. `lib/services/audio/audio_provider.dart`

**修改位置**: `playTemporary()` 方法（约 387 行）和 `_restoreSavedState()` 方法（约 448 行）

应用相同的修改模式。

#### 4. `lib/services/audio/queue_manager.dart`

**修改位置**: `_prefetchTrack()` 方法（约 612 行）和 `ensureAudioUrl()` 方法（约 710 行）

应用相同的修改模式。

---

## 修复 2：统一 `_getDownloadBaseDir` 实现

### 问题描述
4 个文件中有几乎相同的路径获取代码，违反 DRY 原则。

### 修复方案
在 `DownloadPathUtils` 中添加静态方法，其他文件调用该方法。

### 修改文件

#### 1. `lib/services/download/download_path_utils.dart`

**新增方法**:
```dart
import 'dart:io';
import 'package:path/path.dart' as p;
import 'package:path_provider/path_provider.dart';
import '../../data/repositories/settings_repository.dart';

class DownloadPathUtils {
  // ... 现有代码 ...

  /// 获取默认下载基础目录
  /// 
  /// 优先级：
  /// 1. 用户自定义目录（settings.customDownloadDir）
  /// 2. Android: /storage/emulated/0/Music/FMP
  /// 3. Windows: C:\Users\xxx\Documents\FMP
  static Future<String> getDefaultBaseDir(SettingsRepository settingsRepo) async {
    final settings = await settingsRepo.get();
    
    // 1. 优先使用自定义目录
    if (settings.customDownloadDir != null && settings.customDownloadDir!.isNotEmpty) {
      return settings.customDownloadDir!;
    }
    
    // 2. Android: 外部存储 Music 目录
    if (Platform.isAndroid) {
      final extDir = await getExternalStorageDirectory();
      if (extDir != null) {
        final musicDir = p.join(extDir.parent.parent.parent.parent.path, 'Music', 'FMP');
        return musicDir;
      }
      final appDir = await getApplicationDocumentsDirectory();
      return p.join(appDir.path, 'FMP');
    }
    
    // 3. Windows/其他: Documents 目录
    final docsDir = await getApplicationDocumentsDirectory();
    return p.join(docsDir.path, 'FMP');
  }
}
```

#### 2. `lib/services/download/download_service.dart`

**删除**: `_getDefaultDownloadDir()` 方法（596-616 行）

**修改**: 所有调用改为 `DownloadPathUtils.getDefaultBaseDir(_settingsRepository)`

#### 3. `lib/services/import/import_service.dart`

**删除**: `_getDownloadBaseDir()` 方法（489-509 行）

**修改**: 所有调用改为 `DownloadPathUtils.getDefaultBaseDir(_settingsRepository)`

#### 4. `lib/services/library/playlist_service.dart`

**删除**: `_getDownloadBaseDir()` 方法（247-267 行）

**修改**: 所有调用改为 `DownloadPathUtils.getDefaultBaseDir(_settingsRepository)`

#### 5. `lib/services/library/playlist_folder_migrator.dart`

**删除**: `_getDefaultDownloadDir()` 方法（197-210 行）

**注意**: 此类需要添加 `SettingsRepository` 依赖，或者创建一个不需要 settings 的简化版本。

---

## 修复 3：移除 `cachedPath` 字段

### 问题描述
`cachedPath` 字段在代码中被读取但从未被设置，始终为 `null`。

### 修复方案
完全移除该字段及其所有引用。

### 修改文件

#### 1. `lib/data/models/track.dart`

**删除字段**:
```dart
// 删除这一行
String? cachedPath;
```

#### 2. `lib/services/audio/audio_provider.dart`

**修改位置**: 多处使用 `track.cachedPath`

搜索并删除所有 `cachedPath` 引用：
- `_playTrack()`: `trackWithUrl.cachedPath ??` 
- `playTemporary()`: `trackWithUrl.cachedPath ??`
- `_restoreSavedState()`: `trackWithUrl.cachedPath ??`
- `_prepareCurrentTrack()`: `trackWithUrl.cachedPath ??`

#### 3. `lib/services/audio/queue_manager.dart`

**修改位置**: 多处使用 `track.cachedPath`

搜索并删除所有 `cachedPath` 引用：
- `_prefetchTrack()`: `track.cachedPath ??`
- `ensureAudioUrl()`: `track.cachedPath ??`

#### 4. 运行 `build_runner`

由于修改了 Isar 模型，需要重新生成：
```bash
flutter pub run build_runner build --delete-conflicting-outputs
```

---

## 修复 4：`localCoverPath` 同步 I/O

### 问题描述
`localCoverPath` getter 中使用 `File.existsSync()` 会阻塞 UI 线程。

### 建议方案：移除存在性检查

**理由**：
1. `ImageLoadingService.loadImage()` 已经处理了本地文件不存在的情况
2. `LocalImageCache.getLocalImage()` 内部会检查文件是否存在
3. 如果文件不存在，会自动回退到网络图片
4. 无需在 getter 中重复检查

### 修改文件

#### `lib/core/extensions/track_extensions.dart`

**旧代码**:
```dart
String? get localCoverPath {
  if (firstDownloadPath == null) return null;
  final dir = Directory(firstDownloadPath!).parent;
  final coverPath = '${dir.path}/cover.jpg';
  return File(coverPath).existsSync() ? coverPath : null;  // 同步 I/O
}
```

**新代码**:
```dart
/// 获取本地封面路径（基于第一个下载路径计算）
/// 
/// 注意：此方法不检查文件是否存在，由 ImageLoadingService 处理回退逻辑
String? get localCoverPath {
  if (firstDownloadPath == null) return null;
  final dir = Directory(firstDownloadPath!).parent;
  return '${dir.path}/cover.jpg';
}
```

**可选优化**：如果需要检查多个下载路径的封面：
```dart
/// 获取所有可能的本地封面路径
List<String> get possibleCoverPaths {
  return downloadPaths.map((path) {
    final dir = Directory(path).parent;
    return '${dir.path}/cover.jpg';
  }).toList();
}
```

---

## 修复 5：数据源不一致

### 状态：✅ 可接受

**原因**：
- 歌单页面使用数据库是正确的（需要显示歌单内容）
- 已下载页面使用文件扫描是正确的（即使数据库损坏也能访问文件）
- 两者职责不同，不需要统一

---

## 修复 6：统一"正在播放"判断方式

### 问题描述
不同页面使用不同的方式判断当前歌曲是否正在播放。

### 修复方案
统一使用 `sourceId + pageNum` 比较，因为这是最可靠的标识方式。

### 修改文件

#### `lib/ui/pages/library/downloaded_category_page.dart`

**修改位置**: 约 602 行

**旧代码**:
```dart
final isPlaying = currentTrack?.firstDownloadPath == track.firstDownloadPath;
```

**新代码**:
```dart
final isPlaying = currentTrack != null &&
    currentTrack.sourceId == track.sourceId &&
    currentTrack.pageNum == track.pageNum;
```

---

## 执行顺序

建议按以下顺序执行修复：

1. **修复 3**（移除 cachedPath）- 先清理无用代码，减少后续修改量
2. **修复 2**（统一 getDownloadBaseDir）- 重构基础设施
3. **修复 1**（验证文件存在性）- 核心功能修复
4. **修复 4**（localCoverPath）- 性能优化
5. **修复 6**（统一播放判断）- UI 一致性

## 测试要点

### 修复 1 测试
- [ ] 手动删除已下载文件后播放，应回退到网络流
- [ ] 多个歌单都下载了同一首歌，删除一个后应使用另一个

### 修复 2 测试
- [ ] 导入歌单时路径计算正确
- [ ] 下载时路径与导入时一致
- [ ] 自定义下载目录生效

### 修复 3 测试
- [ ] 编译通过
- [ ] 播放功能正常

### 修复 4 测试
- [ ] 图片加载正常
- [ ] 本地封面不存在时显示网络封面

### 修复 6 测试
- [ ] 已下载页面正确高亮当前播放歌曲
