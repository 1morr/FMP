...
- Search "all sources" must load `Settings.enabledSources` at runtime and query only enabled source types; selecting a disabled source should not silently hit that source.
...
- Lyrics popup style fields (`lyricsWindowTextColor`, outline, shadow, opacity, and offsets) are nullable sentinel values; `null` means the built-in default transparent-window style, so no upgrade repair is needed.
- `audioFormatPriority`, `youtubeStreamPriority`, `bilibiliStreamPriority`, `neteaseStreamPriority`
- `enabledSources` (default Bilibili + YouTube + Netease; empty legacy values repair to all direct sources), `rankingRefreshIntervalMinutes`, `radioRefreshIntervalMinutes`
