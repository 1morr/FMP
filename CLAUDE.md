...
- `lyricsAiTitleParsingModeIndex` (default `off`; legacy index `1`/fallback repaired to `off`), `lyricsAiTimeoutSeconds` (default `10s`)
...
AI title parsing can improve auto-match by extracting likely song metadata from noisy titles. Modes: `off`, `alwaysAi`, `advancedAiSelect`; advanced matching is persisted for upcoming wiring but not fully exposed yet. `alwaysAi` means the search-title parsing step uses AI first after direct ID fetches fail or are unavailable; it is not limited to Bilibili/YouTube. Requests send the video/title string plus optional `uploader` context (currently `Track.artist`) to the configured OpenAI-compatible endpoint; `uploader` is explicitly not treated as the song artist. AI returns a track name plus optional artist name; artist is only used when artist confidence is at least `0.8`, otherwise searches use the track name only. Successful parses are stored in `LyricsTitleParseCache` for reuse during the current app run, and the cache is cleared on startup. AI output is used only to generate candidate search terms; actual matching still queries the enabled lyrics sources and saves a normal source-backed `LyricsMatch` only when a source result passes matching rules.
