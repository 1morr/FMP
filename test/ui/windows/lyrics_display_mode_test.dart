...
void main() {
  group('LyricsDisplayMode (C1c)', () {
    test('fromIndex maps the three wire values', () {
      ...
      expect(
        LyricsDisplayMode.fromIndex(1),
        LyricsDisplayMode.preferTranslated,
      );
      expect(LyricsDisplayMode.fromIndex(2), LyricsDisplayMode.preferRomaji);
    });

    test(
      'fromIndex is lenient: out-of-range and null fall back to original',
      () {
    ...
    test(
      'next cycles original → preferTranslated → preferRomaji → original',
      () {
        expect(
          LyricsDisplayMode.original.next,
          LyricsDisplayMode.preferTranslated,
        );
        expect(
