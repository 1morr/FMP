...
void main() {
  group('DownloadService media handoff usage', () {
    test(
      'download service applies image headers to cover and avatar downloads',
      () {
        final source = File(
        ...
        expect(
          source,
          contains('ThumbnailUrlUtils.getOptimizedUrlCandidates('),
        );
        ...
        expect(
          source,
          isNot(
            contains('await _dio.download(track.thumbnailUrl!, coverPath);'),
          ),
        );
        expect(
          source,
          isNot(
            contains('await _dio.download(videoDetail.ownerFace, avatarPath);'),
          ),
        );
      },
    );
    ...
    test(
      'download isolate delegates hop headers and range to MediaHandoff',
      () {
        final source = File(
          'lib/services/download/download_service.dart',
        ).readAsStringSync();
