...
void main() {
  group('AiLyricsSelector', () {
    test('parses selected candidate JSON', () {
      final result = AiLyricsSelector.parseContent(
        jsonEncode({
    ...
    test(
      'sends enriched candidate selection payload without API key leakage',
      () async {
        final dio = Dio();
    ...
    test(
      'omits blank video description and preserves empty lyrics preview',
      () async {
        final dio = Dio();
        Map<String, dynamic>? capturedBody;
        dio.httpClientAdapter = _FakeHttpClientAdapter((options, requestBody) {
