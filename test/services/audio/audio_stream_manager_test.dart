...
void main() {
  ...
  group('AudioStreamManager Task 2 regression', () {
    ...
    setUp(() async {
      tempDir = await Directory.systemTemp.createTemp('audio_stream_manager_');
    ...
    test(
      'selectPlayback passes SourceAuthContext headers to stream fetch',
      () async {
        ...
        expect(
          sourceManager.source.audioStreamRequests.map((r) => r.sourceId),
          ['stream-auth'],
        );
    ...
    test(
      'selectPlayback returns local typed media for downloaded files',
      () async {
    ...
    test(
      'selectPlayback returns remote typed media with prepared headers',
      () async {
    ...
    test(
      ...
      () async {
        ...
        final (updatedTrack, localPath) = await manager.ensureAudioUrl(
          savedTrack,
        );
        ...
        expect(
          queueCopy!.playlistInfo[0].downloadPath,
        ...
        expect(
          persistedTrack.playlistInfo[1].downloadPath,
          firstValidFile.path,
        );
        expect(
          persistedTrack.playlistInfo[2].downloadPath,
    ...
    test(
      ...
      () async {
        ...
        expect(
          media.url.toString(),
          'https://example.com/stream-temporary.m4a',
        );
    ...
    test(
      'default manager emits removed download paths on its stream',
      () async {
    ...
    test(
      'selectPlayback ignores cleanup notifications after manager disposal',
      () async {
    ...
    test(
      'ensureAudioUrl clears missing paths after the first valid local file',
      () async {
    ...
    test(
      ...
      () async {
        ...
        expect(
          persistedTracks.single.playlistInfo.single.downloadPath,
          isEmpty,
        );
      },
    );
    ...
    test(
      'selectPlayback matches persisted track by page when cid is missing',
      () async {
        final firstMissingPath = '${tempDir.path}/missing-ambiguous-first.m4a';
        final secondMissingPath =
            '${tempDir.path}/missing-ambiguous-second.m4a';
        ...
        expect(
          firstPersisted!.playlistInfo.single.downloadPath,
          firstMissingPath,
        );
    ...
    test(
      ...
      () async {
        ...
        final (updatedTrack, localPath, streamResult) = await manager
            .ensureAudioStream(requestTrack!);
    ...
    test('playback headers use shared source media policy', () async {
      ...
      expect(youtube.headers, SourceHttpPolicy.mediaHeaders(SourceIds.youtube));
      expect(
        bilibili.headers,
    ...
    test(
      'selectPlayback attaches playback headers for remote streams',
      () async {
        final selection = await manager.selectPlayback(
    ...
    test(
      'selectPlayback stores source-provided stream expiry on the track',
      () async {
        ...
        final expiryDelta = media.track.audioUrlExpiry!.difference(
          DateTime.now(),
        );
        expect(expiryDelta.inMinutes, inInclusiveRange(15, 16));
      },
    );

    test(
      'selectPlayback falls back to one hour when source omits expiry',
      () async {
        ...
        final expiryDelta = selection.media.track.audioUrlExpiry!.difference(
          DateTime.now(),
        );
        expect(expiryDelta.inMinutes, inInclusiveRange(59, 60));
      },
    );
    ...
    test(
      ...
      () async {
        ...
        expect(selection.media, isA<RemotePlaybackMedia>());
        expect(
          (selection.media as RemotePlaybackMedia).url.toString(),
          'https://example.com/BVmultiPage-24680.m4a',
        );
        expect(
          bilibili.primaryRequests.map(
    ...
    test(
      'selectFallbackPlayback assembles fallback selection with headers',
      () async {
        ...
        expect(media.track.audioUrlExpiry, isNotNull);
        final fallbackExpiryDelta = media.track.audioUrlExpiry!.difference(
          DateTime.now(),
        );
        expect(fallbackExpiryDelta.inMinutes, inInclusiveRange(15, 16));
        expect(media.headers, {
          'Origin': 'https://www.youtube.com',
          'Referer': 'https://www.youtube.com/',
          'User-Agent': AudioStreamManager.defaultPlaybackUserAgent,
        ...
        expect(
          sourceManager.source.lastFailedUrl,
    ...
    test(
      ...
      () async {
        ...
        const failedUrl =
            'https://example.com/handoff-failed-url-exclusion.m4a';
    ...
    test('prepareNetworkPlayback follows the resolved playback URL', () async {
      final neteaseContext = _FakeSourceAuthContext()
        ...
        ..playbackUrlResolver = (sourceType, url, authHeaders) async {
          expect(sourceType, SourceIds.netease);
      ...
      expect(
        ...
        'https://attacker.example/netease-song.m4a',
      );
    ...
    test(
      ...
      () async {
        final accountContext = _FakeSourceAuthContext()
          ..headersBySource[SourceIds.bilibili] = const {
            'Cookie': 'SESSDATA=bilibili-session',
          }
          ..headersBySource[SourceIds.youtube] = const {
        ...
        expect(
          bilibiliMedia.headers,
          SourceHttpPolicy.mediaHeaders(SourceIds.bilibili),
        );
        expect(
          youtubeMedia.headers,
...
class _FakeSourceAuthContext implements SourceAuthContext {
  ...
  final playbackNetworkRequests = <({String sourceType, String url})>[];
...
class _FakeSourceException extends SourceApiException {
  const _FakeSourceException({required this.kind, required this.message});
