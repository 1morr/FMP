...
void main() {
  ...
  test(
    'download resolution skips local files and resolves a remote stream',
    () async {
      final localFile = File('${tempDir.path}/downloaded.m4a');
      await localFile.writeAsString('audio');
      final savedTrack = await trackRepository.save(
      ...
      expect(result, isA<RemoteStreamResolution>());
      expect(
        (result as RemoteStreamResolution).stream.url,
  ...
  test(
    'resolvePrimary applies quality fallback and persists source expiry',
    () async {
      source
      ...
      expect(result, isA<RemoteStreamResolution>());
      expect(
        source.primaryRequests.map((request) => request.config.qualityLevel),
        [AudioQualityLevel.high, AudioQualityLevel.medium],
      );
      expect(
        source.primaryRequests.every((request) => request.cid == 24680),
        isTrue,
      );
      expect(
        source.primaryRequests.every((request) => request.pageNum == 2),
        isTrue,
      );
      final persistedTrack = await trackRepository.getById(savedTrack.id);
  ...
  test(
    'resolvePrimary reuses a still-fresh stream instead of resolving twice',
    () async {
  ...
  test(
    'resolvePrimary re-resolves when the URL is inside the refresh margin',
    () async {
  ...
  test(
    'resolvePrimary re-resolves after the audio quality setting changes',
    () async {
      final first = await service.resolvePrimary(
        _track('quality'),
        purpose: StreamResolutionPurpose.playback,
  ...
  test(
    'invalidateStream forces a fresh resolve after a playback failure',
    () async {
      final first = await service.resolvePrimary(
        _track('invalidate'),
        purpose: StreamResolutionPurpose.playback,
  ...
  test(
    'resolvePrimary does not reuse across pages sharing a sourceId',
    () async {
  ...
  test('resolveFallback passes failedUrl and updates track URL', () async {
    ...
    expect(
      source.alternativeRequests.single.failedUrl,
