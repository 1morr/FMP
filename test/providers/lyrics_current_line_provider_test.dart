...
void main() {
  group('current lyrics line index provider support', () {
    test(
      'calculateCurrentLyricsLineIndex changes only at lyric boundaries',
      () {
    ...
    test('calculateCurrentLyricsLineIndex applies offset', () {
      const lyrics = ParsedLyrics(
        isSynced: true,
        lines: [LyricsLine(timestamp: Duration(seconds: 3), text: 'line')],
      );
    ...
    test(
      'calculateOffsetForLine aligns clicked line start to current position',
      () {
