...
| Phase 4 | 完整 UI | ✅ 已完成 | 2026-02-01 |
| Phase 5 | 平台特性 | ✅ 已完成 | 2026-02-09 |
| Phase 6 | 优化与完善 | 🔄 进行中 | - |

**当前里程碑**: Milestone 3 (可发布) 🔄 进行中
...
|---------|----------|------|--------|------|
| 4.1.1 | 定义响应式断点 | 1.3.7 | P0 | ✅ |
| 4.1.2 | 实现 MobileLayout | 4.1.1 | P0 | ✅ |
| 4.1.3 | 实现 TabletLayout | 4.1.1 | P1 | ✅ |
| 4.1.4 | 实现 DesktopLayout | 4.1.1 | P0 | ✅ |
| 4.1.5 | 实现 ResponsiveScaffold | 4.1.2-4 | P0 | ✅ |

**验收标准**:
- [x] 移动端 (<600dp) 显示底部导航栏
- [x] 桌面端 (>1200dp) 显示侧边栏 + 右侧面板
- [x] 平板端 (600-1200dp) 显示侧边导航
- [x] 窗口大小变化时布局自动切换

> ✅ **Phase 4.1 已完成** - ResponsiveScaffold 已实现，支持移动端、平板端和桌面端布局

### 4.2 主题系统

| 任务 ID | 任务名称 | 依赖 | 优先级 | 状态 |
|---------|----------|------|--------|------|
| 4.2.1 | 定义浅色主题 | 1.2.4 | P0 | ✅ |
| 4.2.2 | 定义深色主题 | 1.2.4 | P0 | ✅ |
| 4.2.3 | 实现主题切换 | 4.2.1-2 | P0 | ✅ |
| 4.2.4 | 实现自定义颜色 | 4.2.3 | P1 | ✅ |
| 4.2.5 | 实现颜色选择器 UI | 4.2.4 | P1 | ✅ |
| 4.2.6 | 创建 ThemeProvider | 4.2.1-5 | P0 | ✅ |
...
> ✅ **Phase 4.2 已完成** - app_theme.dart + themeProvider 已实现
...
|---------|----------|------|--------|------|
| 4.3.1 | 实现首页 | 4.1.5 | P0 | ✅ |
| 4.3.2 | 实现搜索页 | 3.3.6, 4.1.5 | P0 | ✅ |
| 4.3.3 | 实现播放器页 | 2.1.8, 4.1.5 | P0 | ✅ |
| 4.3.4 | 实现迷你播放器 | 4.3.3 | P0 | ✅ |
| 4.3.5 | 实现播放队列页 | 2.2.8, 4.1.5 | P0 | ✅ |
| 4.3.6 | 实现音乐库页 | 3.1.7, 4.1.5 | P0 | ✅ |
| 4.3.7 | 实现歌单详情页 | 4.3.6 | P0 | ✅ |
| 4.3.8 | 实现设置页 | 4.2.6, 4.1.5 | P0 | ✅ |
| 4.3.9 | 实现缓存设置页 | 4.3.8 | P1 | ✅ |
| 4.3.10 | 实现下载管理页 | 4.3.8 | P1 | ✅ |
| 4.3.11 | 实现探索页 | 4.1.5 | P1 | ✅ |
| 4.3.12 | 实现播放历史页 | 4.1.5 | P2 | ✅ |
| 4.3.13 | 实现音频设置页 | 4.3.8 | P1 | ✅ |

**验收标准**:
- [x] 所有核心页面可正常导航
- [x] 播放器页支持手势控制和进度拖拽
- [x] 迷你播放器显示当前歌曲信息和控制按钮
- [x] 队列页支持拖拽排序
- [x] 设置页可修改主题和缓存设置
- [x] 缓存设置支持查看大小、设置上限、清除缓存
- [x] 下载管理页显示下载任务和进度
- [x] 探索页显示排行榜（带缓存）
- [x] 音频设置页支持音质选择

> ✅ **Phase 4.3 已完成** - 所有页面已实现
...
- [x] 应用切到后台后音频继续播放
- [x] 通知栏显示当前歌曲信息和控制按钮
- [x] 通知栏可控制播放/暂停/上下曲 ✅ (2026-01-17 完成)
- [x] 锁屏界面显示播放控制
- [x] 后台播放稳定性测试通过

> ✅ **Phase 5.1 已完成** - Android 后台播放和通知栏控制完整实现

### 5.2 Windows 桌面特性

| 任务 ID | 任务名称 | 依赖 | 优先级 | 状态 |
|---------|----------|------|--------|------|
| 5.2.1 | 实现系统托盘 | 2.1.8 | P0 | ✅ |
| 5.2.2 | 实现托盘右键菜单 | 5.2.1 | P0 | ✅ |
| 5.2.3 | 实现托盘当前歌曲显示 | 5.2.1 | P1 | ✅ |
| 5.2.4 | 实现全局快捷键 | 2.1.8 | P0 | ✅ |
| 5.2.5 | 实现快捷键自定义 | 5.2.4 | P1 | ✅ |
| 5.2.6 | 实现窗口管理 | 5.2.1 | P1 | ✅ |
| 5.2.7 | 实现最小化到托盘 | 5.2.1, 5.2.6 | P1 | ✅ |
| 5.2.8 | 实现 SMTC 媒体控制 | 5.2.1 | P0 | ✅ |

**验收标准**:
- [x] 系统托盘图标显示应用状态
- [x] 右键托盘显示播放控制菜单
- [x] 托盘悬停显示当前歌曲信息
- [x] 全局快捷键可控制播放 (Ctrl+Alt+Space/Left/Right)
- [x] 关闭窗口时最小化到托盘而非退出
- [x] 快捷键可自定义配置
- [x] Windows SMTC 媒体控制集成

> ✅ **Phase 5.2 已完成** - Windows 桌面特性完整实现（托盘、快捷键自定义、窗口管理、SMTC）
...
| 任务 ID | 任务名称 | 依赖 | 优先级 | 状态 |
|---------|----------|------|--------|------|
| 5.4.1 | 实现版本检查服务 | 1.3.1 | P1 | ✅ |
| 5.4.2 | 实现更新下载 | 5.4.1 | P1 | ✅ |
| 5.4.3 | 实现安装引导 | 5.4.2 | P1 | ✅ |
| 5.4.4 | 实现更新提示 UI | 5.4.1 | P1 | ✅ |

**验收标准**:
- [x] 可检查 GitHub Releases 获取最新版本
- [x] 可下载更新包并显示进度
- [x] Android 支持 APK 安装引导
- [x] Windows 支持 MSIX 安装引导
- [x] 设置页显示当前版本和更新按钮

> ✅ **Phase 5.4 已完成** - 应用更新功能完整实现

### 5.5 Mix/Radio 播放

| 任务 ID | 任务名称 | 依赖 | 优先级 | 状态 |
|---------|----------|------|--------|------|
| 5.5.1 | 实现 Mix 播放模式 | 2.2.8 | P1 | ✅ |
| 5.5.2 | 实现 Radio 播放页面 | 5.5.1 | P1 | ✅ |
| 5.5.3 | 实现自动刷新机制 | 5.5.1 | P1 | ✅ |
...
> ✅ **Phase 5.5 已完成** - Mix/Radio 播放功能实现
...
| 任务 ID | 任务名称 | 依赖 | 优先级 | 状态 |
|---------|----------|------|--------|------|
| 6.1.1 | 优化大列表滚动性能 | 4.3.5 | P0 | ✅ |
| 6.1.2 | 实现图片懒加载 | 4.3.1 | P1 | ✅ |
| 6.1.3 | 优化数据库查询 | 1.3.2 | P1 | ✅ |
| 6.1.4 | 减少不必要的重建 | 4.3.1 | P1 | ✅ |
| 6.1.5 | 测量并优化启动时间 | 1.1.1 | P1 | ✅ |

**验收标准**:
- [x] 1000+ 歌曲列表滚动流畅 (60fps) - 分组计算已缓存，文件扫描已异步化
- [x] 图片懒加载，可见区域外不加载（核心页面已使用 ListView.builder/GridView.builder/SliverList）
- [x] 应用冷启动时间优化 - Windows 初始化并行化
- [x] 页面切换无明显卡顿 (UI 刷新性能已优化)

**优化实现说明 (2026-01-31)**:
- ✅ **大列表滚动优化**: `playlist_detail_page.dart` 已使用 `_cachedGroups` 缓存分组计算结果
- ✅ **图片懒加载**: 核心页面均使用 `.builder` 模式 + `ImageLoadingService` + `CachedNetworkImage`
- ✅ **数据库查询优化**: `Track.updatedAt` 添加索引，优化已下载排序查询
- ✅ **减少重建**: Download Provider 已拆分为 `download_state.dart`、`download_scanner.dart`、`file_exists_cache.dart` 等
- ✅ **启动时间优化**: Windows 平台 SMTC 和 WindowManager 初始化并行执行
- ✅ **文件扫描异步化**: `downloadedCategoriesProvider` 使用 `Isolate.run()` 在单独 isolate 中执行

> ✅ **Phase 6.1 已完成** - 所有性能优化任务完成

### 6.2 缓存与下载

| 任务 ID | 任务名称 | 依赖 | 优先级 | 状态 |
|---------|----------|------|--------|------|
| 6.2.1 | 实现 CacheManager | 1.2.4 | P0 | ✅ |
| 6.2.2 | 实现 LRU 自动清理 | 6.2.1 | P0 | ✅ |
| 6.2.3 | 实现 DownloadService | 1.2.6 | P0 | ✅ |
| 6.2.4 | 实现下载进度追踪 | 6.2.3 | P0 | ✅ |
| 6.2.5 | 实现断点续传 | 6.2.3 | P1 | ✅ |
| 6.2.6 | 实现批量下载 | 6.2.3 | P1 | ✅ |

**验收标准**:
- [x] 缓存自动清理超过限制的旧文件 (NetworkImageCacheService.trimCacheIfNeeded)
- [x] 下载任务显示进度百分比 (DownloadProgressEvent + 全局节流)
- [x] 下载中断后可断点续传 (Range header 支持)
- [x] 支持批量下载整个歌单 (addPlaylistDownload)
- [x] 离线模式下可播放已下载歌曲

> ✅ **Phase 6.2 已完成** - 图片缓存管理、下载服务、断点续传、批量下载全部实现
...
- ✅ **文档同步更新**
  - PRD.md 更新至 v1.1.0
  - TECHNICAL_SPEC.md 更新至 v1.1.0
  - WORKFLOW.md 更新至 v1.1.0
...
- ✅ **Phase 6.1 性能优化全部完成**
  - 大列表滚动优化（分组计算缓存 + 文件扫描异步化）
  - 数据库查询优化（Track.updatedAt 添加索引）
  - 启动时间优化（Windows SMTC/WindowManager 并行初始化）
  - Provider 拆分（download Provider 模块化）
  - 文件扫描异步化（downloadedCategoriesProvider 使用 Isolate.run()）

- ✅ Phase 6.2 缓存与下载完整实现
  - NetworkImageCacheService（图片缓存 + LRU 自动清理）
  - DownloadService（完整功能 + 进度追踪）
  - 断点续传（Range header 支持）
  - 批量下载（addPlaylistDownload）
...
| 文件 | 功能 |
|------|------|
| `lib/ui/layouts/responsive_scaffold.dart` | 响应式布局框架 |
| `lib/ui/theme/app_theme.dart` | Material 3 主题系统 |
| `lib/providers/theme_provider.dart` | 主题状态管理 |
| `lib/ui/pages/player/player_page.dart` | 增强版播放器页面 |
| `lib/ui/widgets/player/mini_player.dart` | 迷你播放器 (拖拽进度条) |
| `lib/ui/pages/queue/queue_page.dart` | 播放队列页面 |
| `lib/ui/pages/settings/settings_page.dart` | 设置页面 (主题/缓存) |
| `lib/ui/pages/settings/download_manager_page.dart` | 下载管理页面 |
| `lib/ui/widgets/dialogs/add_to_playlist_dialog.dart` | 添加到歌单对话框 |
| `lib/ui/widgets/track_detail_panel.dart` | 歌曲详情面板 (可展开描述) |
| `lib/ui/widgets/refresh_progress_indicator.dart` | 刷新进度指示器 |
| `lib/ui/widgets/cached_thumbnail.dart` | 带缓存的网络图片组件 |

### Phase 5 实现文件
| 文件 | 功能 |
|------|------|
| `lib/services/audio/audio_handler.dart` | Android 媒体通知控制 |
| `lib/services/audio/queue_manager.dart` | 记住播放位置功能 |
| `lib/services/platform/windows_desktop_service.dart` | Windows 桌面服务（托盘、快捷键、窗口管理） |
| `lib/services/platform/smtc_service.dart` | Windows SMTC 媒体控制服务 |
...
| `lib/services/update_service.dart` | 应用更新服务（版本检查、下载、安装） |
| `lib/ui/pages/settings/update_page.dart` | 更新设置页面 |
| `lib/ui/pages/radio/radio_player_page.dart` | Radio 播放页面 |
| `lib/services/audio/mix_service.dart` | Mix 播放服务 |
| `lib/ui/pages/explore/explore_page.dart` | 探索页面（排行榜） |
| `lib/ui/pages/history/play_history_page.dart` | 播放历史页面 |
| `lib/ui/pages/settings/audio_settings_page.dart` | 音频设置页面（音质选择） |

### Phase 6 实现文件
| 文件 | 功能 |
|------|------|
| `lib/core/constants/app_constants.dart` | 应用常量定义（魔法数字提取） |
| `lib/core/services/network_image_cache_service.dart` | 图片缓存管理（LRU 自动清理） |
| `lib/services/download/download_service.dart` | 下载任务调度（进度追踪、断点续传） |
| `lib/services/download/download_path_manager.dart` | 下载路径选择和管理 |
| `lib/services/download/download_path_utils.dart` | 路径计算工具 |
| `lib/services/download/download_path_sync_service.dart` | 下载路径同步服务 |
| `lib/providers/download/download_scanner.dart` | 下载文件扫描（Isolate 异步化） |
| `lib/providers/download/download_state.dart` | 下载状态定义（Provider 拆分） |
| `lib/providers/download/file_exists_cache.dart` | 文件存在性缓存（减少 UI 重建） |
| `lib/ui/pages/settings/download_manager_page.dart` | 下载管理页面 |
...
- `smtc_windows: ^1.1.0` - Windows SMTC 媒体控制
...
- `dynamic_color: ^1.7.0` - Material You 动态颜色
- `open_file: ^3.5.10` - 文件打开（更新安装）
