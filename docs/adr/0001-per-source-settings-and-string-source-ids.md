...
兩件事在 2026-09 的資料層整理裡一起做，因為它們共用同一批呼叫點：

1. `enum SourceType { bilibili, youtube, netease }` 是封閉的。當時規劃的音源插件化
   需要外掛音源，封閉 enum 擋在那之前。
