...
| 1.2 | 匿名查 cid 改走 `wbi/view` | `lib/data/sources/bilibili_source.dart` | 原定「開機向 `finger/spi` 領 buvid 並持久化」在 2026-09-15 的量測中對任何端點都沒有效果（自造 buvid 與 spi buvid 的結果逐格相同），改為修真正的斷點：匿名 `/x/web-interface/view` 一律 412，`/x/web-interface/wbi/view` 同一請求 200；`playurl` 本身沒問題。同時刪掉「-352 就換指紋重試」（頻率型封鎖，換了也一樣）。驗收：live 測試 Bilibili 不再 skip、模擬器未登入能從搜尋與排行播放 |
| 1.3 | WBI 簽名 | `lib/data/sources/bilibili_source.dart`、`lib/data/sources/source_http_policy.dart` | 2026-09-15 實測 `wbi/view`、`wbi/search/type`、`wbi/playurl` 對未簽、正確簽、錯誤簽、過期 `wts` 全部 200，也就是目前完全不驗簽，做了無法驗證。不做；哪天 live 測試出現 `-403`／簽名相關錯誤再補，演算法以 bilibili-API-collect 的 WBI 文件為準 |
...
| 4.3 | 後端契約測試：三份純規則（`playback_end_reason_rules.dart`、`live_edge_seek_policy.dart`、`next_media_plan.dart`）抽出來由兩個真後端與 `FakeAudioService` 共同轉呼叫，`backend_contract_test.dart` 測規則、`audio_backend_shared_rules_static_rule_test.dart` 釘住只有一份；media_kit 的 `seekToLive` 補上緩衝邊界那一步，fake 不再寫死回 false（已完成） | 加約 200 行測試 | M | 新測試綠；Windows 播 Bilibili 直播 |
| 4.4 | 播放副作用 registry：4 方法介面（onTrackStarted、onPlaybackStateChanged、onStopped、dispose）收 `NowPlayingPublisher`、`PlayHistoryRecorder`、`LyricsAutoMatchCoordinator`，每次呼叫包 try-catch，刪第二個扇出站點（已完成：`_updatePlayingTrack` 成為唯一扇出站點，跟隨路徑與請求成功路徑都不再各自呼叫歌詞自動比對，歌詞改拿解析後的 `trackWithUrl`；`MixSessionCoordinator` 留在 registry 外，它是唯一會讓播放等待的副作用） | 淨 +232 行（新檔 258、控制器 −26），棘輪 2,184 → 2,160 | S–M | 「兩條播放路徑通知同一組消費者」與「teardown 完整」兩條測試 |
...
| 4.9（可選） | 事件路由改成回傳 `sealed class PlaybackAction`、控制器 switch 套用。這 540 行是 #41 #43 #54 #55 的宿主，風險最高，放最後；做之前先照 `5d7dd2da` 的方法量一次。2026-09-16 量過再做（#136）：那段 351 行程式碼引用 51 個成員，13 個是處理器名、16 個純讀、約 20 種副作用；抽成 `playback_event_router.dart` 的快照＋23 個 `PlaybackAction` 變體＋純路由函式後，控制器只從 2,178 減到 2,167 —— 搬走的條件換回一個快照建構子加 23 格 switch。換到的是 50 條純路由斷言與一條「控制器不得再對 `PlaybackEndReason` 做 switch」的靜態規則；實機抓到一個第一版回歸（沒在播時每秒印一行 ignore，改成靜默的 `NothingToDo`）。#41 #43 #54 #55 #106 的六支測試一字未改全綠，Windows 重現 #106 只出一則 toast、無重試 | 棘輪可能下調 300–400 行 → 實際 −11 | M–L | `flutter test test/services/audio` 全綠，兩平台實機 |

4.1、4.2、4.5、4.7 是純刪除，可以直接做。4.3、4.4 加機制，先給設計再做。4.6 動首頁，要實機。

出口：音訊層與音源層合計減 1,000 行以上，後端等價由測試而非口頭保證。（2026-09-16 結算：行數只減 405，等價那半達成；見 §6。）
...
| 5.2 | #92 #93 憑證失效統一呈現 | 產品面的功能，等 M4 之後做；先把 `expiredPlatforms` 這個零讀取的欄位處理掉。2026-09-16：欄位已刪（#131）；#92／#93 本體動到 auth 邊界（失效時保留 `Account` 列改標失效而非 `logout()`），依根 `AGENTS.md` 先給設計，拍板後於 #134 做完：三態帳號卡片、每平台每 session 一次提示、Bilibili 只在刷新也換不回有效 cookie 時才標記、Netease 的 301 只在登入態才算；`logout()` 改成刪列，否則第三態會黏著 |
...
| M4 | 完成 2026-09-16（4.9 於同日補做） | 4.1 #104、4.2 #115、4.7 #116、4.6 #117、4.8 #118、4.5 #119、4.3 #122、4.4 #127；順帶修 #107 #126、#112 #121、#113 #128、#106 #129 | 出口的行數條件**沒有達到**：以 4.1 之前的 `8fbd3517` 為基準，音訊層加音源層的程式碼行 14,453 → 14,048，淨減 405（音源層 −797，音訊層因契約規則檔與 registry 淨增）；整個 `lib` 淨減 1,936 行。後端等價已由 `backend_contract_test.dart` 與靜態規則保證。4.9 事後補做（#136），行數只再減 11，見 4.9 那列；**行數出口就此結案為未達成**，等價與可測性那半達成 |
| M5 | 完成 2026-09-16 | #131、#132；四個外部 repo 各 PR #1；`~/.claude` 的 `5e42449`、`4e47efc` | P-1／P-2 做成行為而非只寫段落，因為寫一條沒有實作的決策正是 5.3 新規則要禁的「文檔宣稱了不存在的東西」；#132 順帶修了備份匯入不蓋 `schemaVersion` 的既有 bug，並把 settings schema 升到 v3、備份格式升到 v5。#92／#93 於 #134 做完並關閉；4.9 於 #136 做完，計劃無待決項 |
