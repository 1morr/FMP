...
- **滑鼠 hover 彈出的 Tooltip（未避開）**
  ...
  - 這就是上游 issue 回報的那一種。它的最小重現是 ListView 裡放兩個 Tooltip，滑鼠從一個移到另一個。2026-09-25 在本機照原樣跑，A、B 來回 4 次就出現 30 行錯誤。只有一個 tooltip 的範本重現不出來：缺的就是「前一個收起、下一個彈出」這一步。
- Slider 那一種沒有最小重現：`flutter create` 的範本加 Slider（第一幀就有、延後出現、放在帶 label 的 Semantics 裡）都是 0 行錯誤，FMP 外殼還有別的條件。上游沒有提到 Slider。
