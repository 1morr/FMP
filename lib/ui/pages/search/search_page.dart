...
import 'package:fmp/ui/widgets/indicators/part_number_badge.dart';
import 'package:fmp/ui/widgets/indicators/source_badge.dart';
import 'package:fmp/ui/widgets/images/radio_cover_image.dart';
import 'package:fmp/ui/widgets/track_group/track_group.dart';
...
class _SearchPageState extends ConsumerState<SearchPage> {
  ...
  Widget build(BuildContext context) {
    ...
    final allTracks = isSelectionMode
        ? [...searchState.localResults, ...searchState.mixedOnlineTracks]
        : const <Track>[];
          ...
          if (state.localResults.isNotEmpty) ...[
            SliverToBoxAdapter(
              child: Padding(
                padding: const EdgeInsets.all(16),
                child: Text(
                  t.searchPage.section.inPlaylist(
                    count: state.localResults.length,
                  ),
                  style: Theme.of(context).textTheme.titleSmall,
                ),
              ),
            ...
            Builder(
              builder: (context) {
                ...
                // 然后按视频分组
                final groupedLocalResults = groupTracks(
                  uniqueTracks.values.toList(),
                );
                return SliverList(
          ...
          if (mixedOnlineTracks.isNotEmpty) ...[
            SliverToBoxAdapter(
              child: Padding(
                padding: const EdgeInsets.all(16),
                child: Text(
                  t.searchPage.section.onlineResults(
                    count: mixedOnlineTracks.length,
                  ),
                  style: Theme.of(context).textTheme.titleSmall,
                ),
              ),
                    ...
                    return Consumer(
                      builder: (context, ref, child) {
                        ...
                        final selectionNotifier = ref.read(
                          searchSelectionProvider.notifier,
                        );
          ...
          child: ListTile(
            ...
            trailing: Row(
              ...
              children: [
                ...
                if (isSelectionMode)
                  _SelectionCheckbox(isSelected: isSelected, onTap: onLongPress)
                else
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return ContextMenuRegion(
      ...
      child: Padding(
        ...
        child: ListTile(
          leading: PartNumberBadge(partNumber: page.page, isPlaying: isPlaying),
          title: Text(
            page.part,
            maxLines: 1,
            overflow: TextOverflow.ellipsis,
    ...
    switch (action) {
      ...
      case addToRemoteTrackActionId:
        final isLoggedIn = ref.read(
          isLoggedInProvider(group.firstTrack.sourceType),
        );
        if (!isLoggedIn) {
          if (context.mounted) {
    ...
    return ContextMenuRegion(
      ...
      child: ListTile(
        ...
        title: Text(
          ...
          maxLines: 1,
          overflow: TextOverflow.ellipsis,
          style: TextStyle(color: room.isLive ? null : colorScheme.outline),
        ),
        subtitle: Row(
          children: [
            // 主播名
