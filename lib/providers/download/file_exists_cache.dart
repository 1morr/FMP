...
class FileExistsCache extends StateNotifier<Map<String, bool>> {
  ...
  /// 注意：对于 content:// URI，如果未缓存则只能返回 null，因为无法同步检测。
  ...
  /// 异步获取第一个存在的下载路径
  ///
  /// 推荐使用此方法替代 getFirstExistingPathSync，支持 content:// URI
  Future<String?> getFirstExistingPathAsync(Track track) async {
    // 先检查缓存
    for (final path in track.downloadPaths) {
      if (state.containsKey(path) && state[path]!) {
        return path;
      }
    }
    ...
    for (final path in track.downloadPaths) {
      if (!state.containsKey(path)) {
        final exists = await _fileExistsService.exists(path);
        updates[path] = exists;
        if (exists) {
          state = {...state, ...updates};
          return path;
...
final fileExistsServiceProvider = Provider<FileExistsService>((ref) {
  return FileExistsService(ref.watch(safServiceProvider));
});

/// 文件存在检查缓存 Provider
final fileExistsCacheProvider =
    StateNotifierProvider<FileExistsCache, Map<String, bool>>((ref) {
  return FileExistsCache(ref.watch(fileExistsServiceProvider));
});
